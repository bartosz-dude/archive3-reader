				...
				<View style={style.container}>
					...
					>
						<View style={style.dataColumn}>
							{q.anyField &&
								<Text>Query: {q.anyField}</Text>
							}
							{
								q.sortDirection &&
								<Text>Order: {q.sortDirection == "desc" ? "descending" : "ascending"}</Text>
							}
						</View>
					</Pressable>
					<View style={style.editColumn}>
						{/* <IconBtn name="pencil-outline" size={32} onPress={() => setMode("edit")} /> */}
						<IconBtn name="delete-outline" size={32} onPress={() => deleteQuery(worksQuery(query)).then(() => router.replace("/search/savedSearches"))} />
		...
		return (
			<>
				<View style={style.container}>
					...
					<View style={style.editColumn}>
						<IconBtn name="close" size={32} onPress={() => setMode("view")} />
						<IconBtn name="delete-outline" size={32} />
					</View>
				</View>
			</>
		)
