...
export default function updateSQLQuery<Data extends [string, unknown][]>(
	table: string,
	data: Data,
	where: [string, SQLValue][],
	dataPreprocessor?: (
		dataEntry: Data[0],
		index: number,
		arr: Data
	...
	const filtered = data.filter((v, i, a) => {
		if (
			!["string", "number", "boolean", "null"].find(
				(type) => typeof v[1] == type
			)
		)
			return
		return v
	...
	filtered.forEach((v, i, a) => {
		...
		if (typeof dataEntry[1] == "string") {
			baseQuery += ` ${dataEntry[0]} = "${dataEntry[1].replace(
				/\"/g,
				'""'
			)}"`
		} else {
