import { AO3WorkSearch, AO3WorkSearchFetch, AO3WorkSearchNumberSearchFields, AO3WorkSearchQuery, CategoriesIds, SortByValues, WarningsIds } from "../types/workSearch";
...
export default function workSearchURL(query: AO3WorkSearchQuery) {
	...
	const workSearch: Omit<AO3WorkSearch, AO3WorkSearchNumberSearchFields> & Partial<Pick<AO3WorkSearch, AO3WorkSearchNumberSearchFields>> = {
		additionalTags: [],
		anyField: "",
		authorArtist: "",
		categories: {},
	...
	const params = new Map<string, string | string[]>([
		...
		[ "archive_warning_ids", (Object.entries(workSearch.warnings) as [ keyof typeof workSearch.warnings, boolean ][]).reduce<string[]>((prev, [ k, v ]) => {
			if (v)
				prev.push(WarningsIds[ k ].toString())
			return prev
		}, []) ],
		[ "category_ids", (Object.entries(workSearch.categories) as [ keyof typeof workSearch.categories, boolean ][]).reduce<string[]>((prev, [ k, v ]) => {
			if (v)
				prev.push(CategoriesIds[ k ].toString())
			return prev
		}, []) ],
