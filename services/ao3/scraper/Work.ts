...
function workScrapper(workId: number, chapter?: number) {
	const htmlStr = testHtml2
	const prunedHtmlStr = (htmlStr).split("</head>")[ 1 ].split("<script")[ 0 ]
	...
	function metaStatParser(type: MetaNumberStat | MetaTextStat): number | string | null {
		...
		if (!stat) {
			throw new Error(`Meta stat of type '${type}' has not been found during parsing`)
		}
		...
		chapters: (() => {
			if (chapter) {
				return [ {
					...
					// @ts-expect-error
					startNotes: htmlParser.DomUtils.textContent(htmlParser.DomUtils
						.findOne((elem) => elem.name == "div" && elem.attribs[ "class" ] == "notes module", dom.children)?.children[ 3 ]).replace(/\r?\n|\r/g, "") ?? "",
					// @ts-expect-error
					endNotes: htmlParser.DomUtils.textContent(htmlParser.DomUtils
						.findOne((elem) => elem.name == "div" && elem.attribs[ "class" ] == "end notes module", dom.children)?.children[ 3 ]).replace(/\r?\n|\r/g, "") ?? "",
					content: htmlParser.DomUtils
						.findOne((elem) => elem.name == "div" && elem.attribs[ "class" ] == "userstuff module", dom.children)
						?.children
						.slice(3, -1)
						// @ts-expect-error
